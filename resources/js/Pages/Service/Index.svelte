<script>
    ...
    export let services; // Son todos los artículos que se encuentran en la base de datos
    let currentPage = services.current_page;

    function create() {
        Inertia.get(route("services.create"));
    }

    function edit(service_id) {
        Inertia.get(route("services.edit", { service: service_id }));
    }
</script>
...
<AdminLayout>
    <Panel class="panel-full">
        <div class="py-2">
            <ButtonPrimary on:click={create}>Nuevo servicio</ButtonPrimary>
        </div>
        <Table>
            <tr>
                <Thead>Título</Thead>
            ...
            <TBody>
                {#each services.data as service}
                    <tr>
                        <TDataCell>{service.title}</TDataCell>
                        <TDataCell>{service.alias}</TDataCell>

                        <TDataCell>{service.created_at}</TDataCell>
                        <TDataCell>{service.updated_at}</TDataCell>
                        <TDataCell>
                            <FormGroup>
                                <ButtonPrimary on:click={() => edit(service.id)}
        ...
        <div class="mt-4">
            <LightPaginationNav
                ...
                on:setPage={(e) => {
                    Inertia.get(
                        route("services.index", { page: e.detail.page })
                    );
